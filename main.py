	...
	def get(self):
		...
		if res:
			if res[1]:
				...
				groups = [(g.name, g.key().id()) for g in query if not bool(list(db.GqlQuery("SELECT * FROM Subscription WHERE user_id = %(user_id)s AND group_id = %(group_id)s"%({"user_id":res[0], "group_id":g.key().id()}))))]
